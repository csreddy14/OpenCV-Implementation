...
def execute(index1, index2, prevH):
    ...
    cv2.imwrite('OutputStitched/l' + str(imageSet) + '/custom/warped_' + str(index2) +  '.png', warpedImage)
...
if __name__ == "__main__":
    ...
    imagePaths = sorted(glob.glob('StitchingImg/I' + str(imageSet) + '/*'))
    os.makedirs('OutputStitched/l' + str(imageSet) + '/custom/', exist_ok = True)
    ...
    finalImg =  cv2.imread('OutputStitched/l' + str(imageSet) + '/custom/'  + 'warped_' + str(0) + '.png')
    if imageSet == 1:
        length = 6
    else:
        length = 5
    for index in range(1, length):
        ...
        img2 = cv2.imread('OutputStitched/l' + str(imageSet) + '/custom/' + 'warped_' + str(index) + '.png')
        # print('blending', index, 'OutputStitched/l' + str(imageSet) + '/custom/' + 'warped_' + str(index) + '.png')
        finalImg, mask1truth, mask2truth = b.blend(finalImg, img2)
        mask1truth = mask1truth + mask2truth
        cv2.imwrite('OutputStitched/l' + str(imageSet) + '/custom/' 'FINALBLENDED.png', finalImg)
